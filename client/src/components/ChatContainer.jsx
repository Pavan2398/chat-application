...
const ChatContainer = () => {
  ...
  const handleAddReaction = async (emoji) => {
    ...
    try {
      ...
      if (data.success) {
        setMessages((prev) => prev.map((msg) => msg._id === showReactionMenu ? data.updatedMessage : msg));
      }
    } catch (error) {
    ...
    <div className="flex flex-col bg-gray-900 h-full overflow-hidden">
      ...
      <div ref={chatBoxRef} onScroll={() => { const el = chatBoxRef.current; if (el && el.scrollTop <= 50 && !isLoadingMore && hasMore && !hasSearch) loadMessagesPage(page + 1, { prepend: true }); }} className="flex-1 overflow-y-auto overflow-x-hidden p-3 flex flex-col">
        ...
        {messages.map((msg, index) => {
          ...
          return (
            <div key={msgId} ref={(el) => { if (el) messageRefs.current[msgId] = el; }} className={`flex items-end gap-1 mb-1 ${isMine ? "justify-end" : "justify-start"}`}>
              ...
              <div className="flex flex-col items-start gap-0.5 max-w-[70%]">
                ...
                <div className="relative group">
                  {msg.deleted ? (
                    ...
                    <img src={msg.image} alt="Shared" className={`max-w-[180px] rounded-2xl ${isMine ? "border border-gray-700" : "border border-gray-200 bg-white"}`} />
                    ...
                    <div className={`p-2 text-sm rounded-2xl break-all ${isMine ? "bg-teal-700 text-white rounded-br-none" : "bg-white text-slate-900 rounded-bl-none shadow-sm"} ${isActiveMatch ? "ring-2 ring-yellow-300" : isMatch ? "ring-1 ring-yellow-200" : ""}`}>
                  ...
                  {!isMine && !msg.deleted && (
                    <button type="button" onClick={(e) => { e.stopPropagation(); setShowReactionMenu(showReactionMenu === msgId ? null : msgId); }} className="absolute -left-8 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-black/50 p-1 rounded-full">
                      <Smile size={14} />
                    </button>
                  )}
                  {showReactionMenu === msgId && !isMine && (
                    <div className="absolute left-0 top-8 z-10 flex gap-1 bg-slate-900 p-1 rounded-xl shadow-xl">
                      {reactionEmojis.map((emoji) => (
                        <button key={emoji} type="button" onClick={(e) => { e.stopPropagation(); handleAddReaction(emoji); setShowReactionMenu(null); }} className="p-1 hover:bg-slate-800 rounded text-lg">{emoji}</button>
                      ))}
                    </div>
                  )}
                </div>
